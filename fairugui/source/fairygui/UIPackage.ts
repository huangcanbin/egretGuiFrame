...
module fairygui
{
	...
	export class UIPackage
	{
		...
		private loadPackage(): void
		{
			...
			var buf: any = Resource.getRes(this._resKey);
			if (!buf)
				buf = Resource.getRes(this._resKey + ".fui");
			if (!buf)
			...
			switch (item.type)
			{
				...
				case PackageItemType.Atlas:
					if (!item.decoded)
					{
						...
						let url:string = this._resKey;
						let regex: RegExp = /^.+\.fui$/;
						if (regex.test(url))
							url = url.substring(0, url.lastIndexOf(".fui"));
						
						item.texture = Resource.getRes(url + "@" + fileName);
						if (!item.texture)
							item.texture = Resource.getRes(url + "@" + fileName.replace("\.", "_"));
					}
					return item.texture;
				...
				case PackageItemType.Sound:
					if (!item.decoded)
					{
						...
						// let name: string = ToolSet.getFileName(fileName);
						item.sound = Resource.getRes(this._resKey + "@" + fileName);
						if (!item.sound)
							item.sound = Resource.getRes(this._resKey + "@" + fileName.replace("\.", "_"));
					}
					return item.sound;
				...
				default:
					return Resource.getRes(this._resKey + "@" + item.id);
			}
		}
