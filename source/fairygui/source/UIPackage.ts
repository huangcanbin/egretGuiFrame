...
module fairygui
{
	...
	export class UIPackage
	{
		...
		{
			let pattern: RegExp = /^(.+?)(\.fui)?$/;
			let results = pattern.exec(this._resKey);
			let head: string = results[1];
			switch (item.type)
			{
				case PackageItemType.Image:
					if (!item.decoded)
				...
				case PackageItemType.Atlas:
					if (!item.decoded)
					{
						...
						item.texture = Resource.getRes(head + "@" + fileName);
						if (!item.texture)
							item.texture = Resource.getRes(head + "@" + fileName.replace("\.", "_"));
					}
					return item.texture;
				...
				case PackageItemType.Sound:
					if (!item.decoded)
					{
						...
						// let name: string = ToolSet.getFileName(fileName);
						item.sound = Resource.getRes(head + "@" + fileName);
						if (!item.sound)
							item.sound = Resource.getRes(head + "@" + fileName.replace("\.", "_"));
					}
					return item.sound;
				...
				default:
					return Resource.getRes(head + "@" + item.id);
			}
		}
